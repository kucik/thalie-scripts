...
void main()
{
    ...
    int sCheck1 = GetLocalInt(OBJECT_SELF,"sCheck1");
    object oPC = GetNearestCreature(CREATURE_TYPE_PLAYER_CHAR,PLAYER_CHAR_IS_PC,OBJECT_SELF);
    if((GetDistanceBetween(OBJECT_SELF,oPC)<=3.5) && (sCheck1!=1))
    {
        ChangeToStandardFaction(OBJECT_SELF,STANDARD_FACTION_HOSTILE);
        SetLocalInt(OBJECT_SELF,"sCheck1",1);
        ActionAttack(oPC);
