...
///  string sBranchTag = GetLocalString(GetModule(), sTreeTag + "_BranchTag");
  string sBranchTag = GetLocalString(OBJECT_SELF,"drevo");
   ...
   if (sBranchTag != "")
   {
    ...
    {
      if (CNR_FLOAT_WOOD_MINING_TREE_RESPAWN_TIME_SECS > 10.0)  //100sekund na respawn
      {
    ...
    DestroyObject(OBJECT_SELF);
    CreateObject(OBJECT_TYPE_PLACEABLE, GetResRef(OBJECT_SELF), locTree);
  }
}
