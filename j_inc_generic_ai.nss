...
int AI_ActionHealUndeadObject(object oTarget)
{
    ...
    if (!bPoly)//...
    {
        ...
        if(AI_ActionCastSpell(SPELL_CIRCLE_OF_DOOM, SpellHostRanged, oTarget, i15, TRUE, ItemHostRanged)) return TRUE;
...
int AI_SpellWrapperShieldProtections(int iLowest = 1)
{
    ...
    {
        // Elemental Shield. Level 4 (Mage). Does some damage to melee attackers (Casterlvl + d6(), Fire). +50% cold/fire resistance.
        if(AI_ActionCastSpell(SPELL_ELEMENTAL_SHIELD, SpellProSelf, OBJECT_SELF, i14, FALSE, ItemProSinTar, PotionPro)) return TRUE;
        if(iLowest <= i3)
        {
            // Acid Sheath. Level 3 (Mage) Does some damage to melee attackers (2 * CasterLvl + 1d6, Acid).
            if(AI_ActionCastSpell(SPELL_MESTILS_ACID_SHEATH, SpellProSelf, OBJECT_SELF, i13, FALSE, ItemProSinTar, PotionPro)) return TRUE;

            // Wounding Whispers. Level 3 (bard) Does some damage to melee attackers (Casterlvl + d6(), Sonic).
            if(AI_ActionCastSpell(SPELL_WOUNDING_WHISPERS, SpellProSelf, OBJECT_SELF, i13, FALSE, ItemProSinTar, PotionPro)) return TRUE;

            if(iLowest <= i2)
            {
                // Death Armor. Level 2 (Mage) Does some damage to melee attackers (Caster Level/2 (to +5) + 1d4, Magical)
                if(AI_ActionCastSpell(SPELL_DEATH_ARMOR, SpellProSelf, OBJECT_SELF, i13, FALSE, ItemProSinTar, PotionPro)) return TRUE;
            }
        }
    }
    return FALSE;
...
//        SendMessageToPC(GetFirstPC(), "Level8");//...
        ...
        if(RangeShortValid && d10() <= i8) // No GlobalNormalSpellsNoEffectLevel check
        {
            ...
            if(!AI_GetSpellTargetImmunity(GlobalImmunityPetrify))
            {
                if(AI_ActionCastSpellRandom(SPELLABILITY_BREATH_PETRIFY, FALSE, i40, GlobalSpellTarget, FALSE, TRUE)) return TRUE;
...
int AI_AttemptBeholderCombat()
{
    ...
    if(!GetLocalInt(OBJECT_SELF, "JA_AI_NOSPECIAL")){
        iBeholderRay = AI_SPELLABILITY_BEHOLDER_MAGIC_CONE;
    }
    else{ //50%:50%
        if(d100() < 50){
            iBeholderRay = AI_SPELLABILITY_BEHOLDER_MAGIC_CONE;
            ...
            iBeholderRay = AI_SPELLABILITY_BEHOLDER_ALLRAYS;
        }
    }
    ...
    if(d100() <= i80 && GlobalSeenSpell)
    {
        ...
        ActionCastSpellAtObject(iBeholderRay, GlobalSpellTarget, METAMAGIC_ANY, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
        return TRUE;
    }
