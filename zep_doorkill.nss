...
void main()
{
    ...
    if (nIsOpen == 0)   //if the door is closed
    {
        ...
        if (GetLocalObject(oSelf, "DOORWAY_BREAK")!= OBJECT_INVALID)
        {
//            SpeakString("Destroying doors break "+GetName(GetLocalObject(oSelf, "DOORWAY_BREAK")));
            DestroyObject(GetLocalObject(oSelf, "DOORWAY_BREAK"));
        }
    }
