...
void main()
{
    ...
    if(sSpecial != ""){
        ExecuteScript(sSpecial, OBJECT_SELF);
    }
       ...
       !GetObjectSeen(oEnemy))  // Nearest enemy is not seen.
    {
        ...
        if( (GetWalkCondition(NW_WALK_FLAG_CONSTANT)) && (NoWalk!=1) )
        {
        ...
        else
        {
            ...
            // longer looping one
            // - Need a valid player.
            if( (GetIsObjectValid(oPlayer)) && (GetLocalInt(OBJECT_SELF,"AI_NOWALK")!=1) )
            {
                ...
                {
                   if( (GetLocalInt(GetLocalObject(OBJECT_SELF,"KU_ACT_AREA"),"KU_LOC_ALIVE") > 0) && (NoWalk!=1)) {
                     /* Srackovy chozeni a animace */
