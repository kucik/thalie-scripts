    ...
    while (GetIsObjectValid(oTarget))
    {
        ...
        if (GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD )
        {
            if(!GetIsReactionTypeFriendly(oTarget))
            {
                ...
                {
                    nModify = d8(nCasterLvl/2);
                    //Make metamagic check
                    if (nMetaMagic == METAMAGIC_MAXIMIZE)
                    {
                        nModify = 8*(nCasterLvl/2);
                    }
                    if (nMetaMagic == METAMAGIC_EMPOWER)
                    {
                        nModify = nModify + (nModify/2)+nCasterLvl; //Damage/Healing is +50%
                    }

                    //Make Fort save
                    if (MySavingThrow(SAVING_THROW_FORT, oTarget, GetSpellSaveDC()+GetThalieSpellDCBonus(OBJECT_SELF), SAVING_THROW_TYPE_NONE, OBJECT_SELF, fDelay))
                    {
                        nModify /= 2;
                    }
