...
/*
    ...
    levels an ability boost (CON). Added metamagic
...
*/
    ...
    eStr = EffectAbilityIncrease(ABILITY_CONSTITUTION, nModify);
    SetEffectSpellId(eStr,SPELL_ENDURANCE);
    eLink = EffectLinkEffects(eStr, eDur);
    SetEffectSpellId(eLink,SPELL_ENDURANCE);
    ...
    while(GetIsObjectValid(oTarget) && nTargets != 0)
    {
        if(GetIsFriend(oTarget) && OBJECT_SELF != oTarget)
        {
            ...
            DelayCommand(fDelay,SignalEvent(oTarget, EventSpellCastAt(oTarget, SPELL_ENDURANCE, FALSE)));
    ...
    DelayCommand(0.1,SignalEvent(oTarget, EventSpellCastAt(oTarget, SPELL_ENDURANCE, FALSE)));
