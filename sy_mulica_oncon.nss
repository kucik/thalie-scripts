...
//:://////////////////////////////////////////////
/*
  modifikovane Sylmaelom
  teraz sa daju vykonat akcie stoj a nasleduj
  ostatne su zakazane
*/
...
void main()
{
    ...
    if (nMatch == -1) {
        ...
        {
            SetCommandable(TRUE);
            //reaguje len na follow,stand
            if ((nMatch==ASSOCIATE_COMMAND_FOLLOWMASTER) || (nMatch==ASSOCIATE_COMMAND_STANDGROUND))
            {
                bkRespondToHenchmenShout(oShouter, nMatch, oIntruder, TRUE);
            } //else return;
            //FloatingTextStringOnCreature("akcia="+IntToString(nMatch),oShouter,FALSE);
        }
    }
