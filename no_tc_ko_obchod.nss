...
// Buï otevøi obchod s tímto tagem, nebo uživateli oznam, že žádný obchod neexistuje.
    ...
    object oStore = GetObjectByTag("no_tcob_ko_" + GetLocalString(OBJECT_SELF,"no_obchod"));
...
oStore = CreateObject(OBJECT_TYPE_STORE,"no_tcob_ko_kara",no_lokace,FALSE,("no_tcob_ko_" + GetLocalString(OBJECT_SELF,"no_obchod")));
        OpenStore(oStore,GetPCSpeaker());
...
if   (GetTag(no_Item)=="no_louh_lege" )  DestroyObject(no_Item);
if   (GetTag(no_Item)=="no_louh_velm" )  DestroyObject(no_Item);
...
if   (GetTag(no_Item)=="no_louh_lege" )  DestroyObject(no_Item);
if   (GetTag(no_Item)=="no_louh_velm" )  DestroyObject(no_Item);
if   (GetTag(no_Item)=="no_louh_mist" )  DestroyObject(no_Item);
if   (GetTag(no_Item)=="no_louh_kval" )  DestroyObject(no_Item);
no_Item = GetNextItemInInventory(oStore);
} }
