...
if ( (GetStringLeft(GetTag(no_Item),3) == "no_")&(GetLocalInt(no_Item,"no_OCAROVAVAM") == TRUE) )
   {
   if ( NO_oc_DEBUG == TRUE )  SendMessageToPC(no_oPC,"no_polotovar tag=" + GetTag(no_Item) );
   if ( NO_oc_DEBUG == TRUE )  SendMessageToPC(no_oPC,"no_polotovar jmeno=" + GetName(no_Item) );
...
if (no_provedeni_polotovaru == FALSE) no_kamen(OBJECT_SELF,FALSE);
...
if (  ( no_menu == 1 ) & (GetLocalString(OBJECT_SELF,"no_vyrobek")!="") & (GetLocalInt(OBJECT_SELF,"no_kamen")!=0) )
{
