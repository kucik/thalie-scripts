...
const string FILE_HEARTBEAT_ANIMATIONS              = "j_ai_heart_aimat";
...
void SafeRest(){
    if( !GetIsDMPossessed(OBJECT_SELF) && !(GetIsFighting() || GetIsInCombat()) ){
        ClearAllActions();
            // 71: "[Search] Resting"
...
void Search(object oTarget = OBJECT_INVALID)
{
    ...
    else
    {
        ...
        ClearAllActions();
        // Rest after combat?
        if(GetSpawnInCondition(AI_FLAG_OTHER_REST_AFTER_COMBAT, AI_OTHER_MASTER) && !GetIsDMPossessed(OBJECT_SELF))
        {
            DelayCommand(300.0f, SafeRest());
        }
        ...
        else
        {
            ...
            if(iSearch != DETECT_MODE_ACTIVE && !GetHasFeat(FEAT_KEEN_SENSE) &&
                (GetSkillRank(SKILL_LISTEN) >= 15 || GetSkillRank(SKILL_SEARCH) >= 15) && d2() == 0
            )
            {
