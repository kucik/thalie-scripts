...
/************************ [User] **********************************************/
    ...
    DelayCommand(f2, SpawnWalkWayPoints());
       ...
       //if(!iHostile){
/*        if( !GetLocalInt(OBJECT_SELF, "AI_NOANIMATIONS") && GetLocalString(OBJECT_SELF, "AI_SITONSPAWN")=="" ){
            if( GetLocalInt(OBJECT_SELF, "AI_NOWALK") ){
                SetSpawnInCondition(NW_FLAG_IMMOBILE_AMBIENT_ANIMATIONS, NW_GENERIC_MASTER);
                // This will cause an NPC to use common animations it possesses,
                // and use social ones to any other nearby friendly NPCs.
            }
...
*/
   ...
   // Treasure generating.
    ...
    if( ai_level != "" ){
       int l = AI_LEVEL_DEFAULT;
       if( ai_level == "AI_LEVEL_VERY_LOW" ) l = AI_LEVEL_VERY_LOW;
       else
       if( ai_level == "AI_LEVEL_LOW" ) l = AI_LEVEL_LOW;
       else
       if( ai_level == "AI_LEVEL_NORMAL" ) l = AI_LEVEL_NORMAL;
       else
       if( ai_level == "AI_LEVEL_HIGH" ) l = AI_LEVEL_HIGH;
       else
       if( ai_level == "AI_LEVEL_VERY_HIGH" ) l = AI_LEVEL_VERY_HIGH;
...
void main()
{
 ...
 SetLocalLocation(OBJECT_SELF,"ku_spawn_loc",GetLocation(OBJECT_SELF));
 SetLocalInt(OBJECT_SELF,"ku_loc_return",TRUE);

 //Igor nahodne boosty
 DelayCommand(1.0,ExecuteScript("ig_boost_npc",OBJECT_SELF));

 //druidi leceni
 if((GetRacialType(OBJECT_SELF)) == RACIAL_TYPE_ANIMAL) {
     DelayCommand(1.0,ExecuteScript("ig_animal_ill",OBJECT_SELF));
 }
//druidi leceni
 ...
 if (GetIsObjectValid(oMaster))
 {
    ...
    if (iEnhant > 0)
    {
        ...
        {
            AddItemProperty(DURATION_TYPE_TEMPORARY, ip,oMyWeapon);
        }
    ...
    ApplyEffectToObject(DURATION_TYPE_PERMANENT,EffectConcealment(iConceal),OBJECT_SELF);
    //Immunity
    int iImm;
    if (iSDLevel>10)
    {
    ...
    ApplyEffectToObject(DURATION_TYPE_PERMANENT,EffectDamageImmunityIncrease(DAMAGE_TYPE_BLUDGEONING,iImm),OBJECT_SELF);
    ApplyEffectToObject(DURATION_TYPE_PERMANENT,EffectDamageImmunityIncrease(DAMAGE_TYPE_SLASHING,iImm),OBJECT_SELF);
    //Onhit
