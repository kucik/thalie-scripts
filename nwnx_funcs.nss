...
int SetGoldPieceValue (object oItem, int nValue) {
    ...
    // Kucik hack
    int iret = NWNXFuncsOne(oItem, "NWNX!FUNCS!SETGOLDPIECEVALUE", nValue);
    ...
    if(iNewPrice != nValue) {
      NWNXFuncsOne(oItem, "NWNX!FUNCS!SETGOLDPIECEVALUE", 0);
      int iNullPrice = GetGoldPieceValue(oItem) / iStack;
      NWNXFuncsOne(oItem, "NWNX!FUNCS!SETGOLDPIECEVALUE", nValue - iNullPrice);
    }
    return iret;
...
int SetItemWeight (object oItem, int nTenthLbs) {
    SetLocalInt(oItem,"WEIGHT",nTenthLbs);
