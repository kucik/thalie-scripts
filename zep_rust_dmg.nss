        ...
        if (GetItemInSlot(INVENTORY_SLOT_RIGHTHAND, oPC) != OBJECT_INVALID)
        {
          ...
          if (!GetWeaponRanged(oWeapon) && (IsItemMetal(oWeapon)>0))
          {
            ...
            if(ReflexSave(oPC,20))// saved
            {
              if (GetIsPC(oPC))
              {
                sMessageToPC=GetStringByStrRef(nZEPRustYourTXT,GENDER_MALE);
                sMessagePart2=GetStringByStrRef(nZEPRustWeapRes,GENDER_MALE);
                SendMessageToPC(oPC,sMessageToPC+sWeapon+sMessagePart2);
              }
            }
            else //didn't save
            {
              ...
              {
                sMessageToPC=GetStringByStrRef(nZEPRustYourTXT,GENDER_MALE);
                sMessagePart2=GetStringByStrRef(nZEPRustWeapDest,GENDER_MALE);
                SendMessageToPC(oPC,sMessageToPC+sWeapon+sMessagePart2);
              }
