...
{
    int nBonus = ArcaneArcherCalculateBonus()+ GetLevelByClass(CLASS_TYPE_ARCANE_ARCHER);
    int nDamage;
    // * Roll Touch Attack
    int nTouch = TouchAttackRanged(oTarget, TRUE);
    if (nTouch > 0)
    {
        nDamage = ArcaneArcherDamageDoneByBow(nTouch ==2)+GetLevelByClass(CLASS_TYPE_ARCANE_ARCHER);
        if (nDamage > 0)
        {
