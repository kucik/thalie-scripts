...
int no_xpy = TC_getXP(no_oPC,31);
// nastavy xp postavy v craftu iCraftID
//void TC_setXP(object oPC, int iCraftID ,int nXP)

TC_setXP(no_oPC,31 ,no_xpy  - 500);
int no_level =  TC_getLevel(no_oPC,31);

    SendMessageToPC(no_oPC ,"Odebrano 500 craftovych xp v krejcovine");
    SendMessageToPC(no_oPC , "V krejcovine si na " + IntToString(no_level) + "urovni.");
    SendMessageToPC(no_oPC ,"Celkem jsi ziskal " + IntToString(no_xpy  - 500) + " XP" );
SetCustomToken(9028, "Krejcovina:" + IntToString(no_xpy -500) + " xp = " + IntToString(no_level) + ". lvl");

SetPersistentInt(no_oPC,"tcXPSystem" + IntToString(31),no_xpy - 500,0);
