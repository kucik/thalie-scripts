...
string ITEM_CASTING()
{
    string sMsg = "Použita vrozená schopnost: Žádný UMD hod.";
...
{
    string sMsg = "UMD hod: Použití vrozené schopnosti - NEÚSPÌCH";
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
*/
void CreateItemInInventory(string sItemTemplate, object oTarget=OBJECT_SELF, int nStackSize=1, string sNewTag=""){
    CreateItemOnObject(sItemTemplate, oTarget, nStackSize, sNewTag);
}

string NO_VALID_SKILL()
{
    string sMsg = "Tento svitek nemuzes pouzit.";
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
{
    string sMsg = GetName(oFam)+" sdílí kouzlo se svým pánem.";
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
{
    string sMsg = "ÚSPÌCH svitek: (UMD skill total + roll) = " +IntToString (iUMDSkillLevel) + " + " + IntToString (ROLL) + " = " +IntToString (iUMDSkillLevel+ROLL) + "   Against DC: " + IntToString (iDC_CRAFT);
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
{
    string sMsg = "KRITICKÝ NEÚSPÌCH svitek: (UMD skill total + roll) = " +IntToString (iUMDSkillLevel) + " + " + IntToString (ROLL) + " = " +IntToString (iUMDSkillLevel+ROLL) + "   Against DC: " + IntToString (iDC_CRAFT);
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
{
    string sMsg = "NEÚSPÌCH svitek: (UMD skill total + roll) = " +IntToString (iUMDSkillLevel) + " + " + IntToString (ROLL) + " = " +IntToString (iUMDSkillLevel+ROLL) + "   Against DC: " + IntToString (iDC_CRAFT);
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
{
    string sMsg = "ÚSPÌCH pøedmìt:: (UMD skill total + roll) = " +IntToString (iUMDSkillLevel) + " + " + IntToString (ROLL) + " = " +IntToString (iUMDSkillLevel+ROLL) + "   Against DC: " + IntToString (iDC_CRAFT);
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
{
    string sMsg = "KRITICKÝ NEÚSPÌCH pøedmìt: (UMD skill total + roll) = " +IntToString (iUMDSkillLevel) + " + " + IntToString (ROLL) + " = " +IntToString (iUMDSkillLevel+ROLL) + "   Against DC: " + IntToString (iDC_CRAFT);
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
{
    string sMsg = "NEÚSPÌCH pøedmìt: (UMD skill total + roll) = " +IntToString (iUMDSkillLevel) + " + " + IntToString (ROLL) + " = " +IntToString (iUMDSkillLevel+ROLL) + "   Against DC: " + IntToString (iDC_CRAFT);
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
{
    string sMsg = "ÚSPÌCH: (spellcraft skill total + roll) = " +IntToString (iUMDSkillLevel) + " + " + IntToString (ROLL) + " = " +IntToString (iUMDSkillLevel+ROLL) + "   Against DC: " + IntToString (iDC_CRAFT);
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
{
    string sMsg = "KRITICKÝ NEÚSPÌCH: (spellcraft skill total + roll) = " +IntToString (iUMDSkillLevel) + " + " + IntToString (ROLL) + " = " +IntToString (iUMDSkillLevel+ROLL) + "   Against DC: " + IntToString (iDC_CRAFT);
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
{
    string sMsg = "NEÚSPÌCH: (spellcraft skill total + roll) = " +IntToString (iUMDSkillLevel) + " + " + IntToString (ROLL) + " = " +IntToString (iUMDSkillLevel+ROLL) + "   Against DC: " + IntToString (iDC_CRAFT);
    if (DM_ALERT) SendMessageToAllDMs(sMsg);
    return sMsg;
}
...
//declarations
    ...
    sTag = GetSubString(GetTag(oSpellItem),0,3);
    ...
    if (!UMD_OTHER)   //do not UMD things other than scrolls and wands...
        {
        if ((GetBaseItemType(oSpellItem)!= BASE_ITEM_SCROLL) &&
           (GetBaseItemType(oSpellItem)!= BASE_ITEM_SPELLSCROLL) &&
           (GetBaseItemType(oSpellItem)!= BASE_ITEM_ENCHANTED_SCROLL) &&
           //(GetBaseItemType(oSpellItem)!= BASE_ITEM_MAGICROD) &&
           (GetBaseItemType(oSpellItem)!= BASE_ITEM_MAGICSTAFF) &&
           (GetBaseItemType(oSpellItem)!= BASE_ITEM_MAGICWAND))
                {
                nOther = TRUE;
                //SendMessageToPC(OBJECT_SELF, ITEM_CASTING());
                }
    ...
    if (
        ...
        (!GetIsObjectValid(oSpellItem)) || // do not run for spells cast by the character
        (GetBaseItemType(oSpellItem)==BASE_ITEM_POTIONS)||// do not run for potions
        ...
        if(
           (GetBaseItemType(oSpellItem) == BASE_ITEM_SCROLL) ||
           (GetBaseItemType(oSpellItem) == BASE_ITEM_ENCHANTED_SCROLL) ||
           (GetBaseItemType(oSpellItem) == BASE_ITEM_SPELLSCROLL)
           )
           {
       ...
       if (!bSUCCESS)
        ...
        if (bUSE_UMD)
            ...
            if (bIS_SCROLL)
                ...
                if (iFINAL_ROLL >= (UMD_DC + iUMD_CRAFT))
                    ...
                    if ((iFINAL_ROLL + UMD_PENALTY_DC) >= (UMD_DC + iUMD_CRAFT))
                        ...
                        FloatingTextStringOnCreature("*Neuspech*", OBJECT_SELF, FALSE);
                        }
                        else
                        {
                        ...
                        FloatingTextStringOnCreature("*Kriticky neuspech*", OBJECT_SELF, FALSE);
                        }
                    }
                 }
                 else
            ...
            if ((iFINAL_ROLL) >= (CRAFT_DC + iUMD_CRAFT))
                ...
                //FloatingTextStringOnCreature("*Neuspech*", OBJECT_SELF, FALSE);
                string sRes = GetResRef(oSpellItem);
                DelayCommand(1.0f, CreateItemInInventory(sRes));
                }
            }
