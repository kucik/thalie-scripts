...
// Buï otevøi obchod s tímto tagem, nebo uživateli oznam, že žádný obchod neexistuje.
    ...
    object oStore = GetObjectByTag("no_tcob_ke_" + GetLocalString(OBJECT_SELF,"no_obchod"));
...
oStore = CreateObject(OBJECT_TYPE_STORE,"no_tcob_ke_kara",no_lokace,FALSE,("no_tcob_ke_" + GetLocalString(OBJECT_SELF,"no_obchod")));
        OpenStore(oStore,GetPCSpeaker());
...
if   (GetTag(no_Item)=="no_slin_velk" )  DestroyObject(no_Item);
if   (GetTag(no_Item)=="no_slin_zahn" )  DestroyObject(no_Item);
...
if   (GetTag(no_Item)=="no_slin_velk" )  DestroyObject(no_Item);
if   (GetTag(no_Item)=="no_slin_zahn" )  DestroyObject(no_Item);
if   (GetTag(no_Item)=="no_slin_stre" )  DestroyObject(no_Item);
if   (GetTag(no_Item)=="no_slin_ampu" )  DestroyObject(no_Item);
no_Item = GetNextItemInInventory(oStore);
} }
