...
// End of Spell Cast Hook
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if(!MyResistSpell(OBJECT_SELF, oTarget))
        {
            ...
            if (!MySavingThrow2(SAVING_THROW_WILL,  oTarget, GetSpellSaveDC()+GetThalieSpellDCBonus(OBJECT_SELF), SAVING_THROW_TYPE_MIND_SPELLS))
            {


                         ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(d6(5)), oTarget);
                ...
                {
                    //Make a Fort save
                    if (MySavingThrow(SAVING_THROW_FORT, oTarget, GetSpellSaveDC()+GetThalieSpellDCBonus(OBJECT_SELF),SAVING_THROW_TYPE_DEATH))
                    {
                         //Check for metamagic
