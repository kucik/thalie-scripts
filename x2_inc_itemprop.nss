...
int IPGetIsMeleeWeapon(object oItem)
{
    ...
    if(GetStringLength(sret) == 1 && sret != "0" ) {
      // Except ranged weapons
      if(Get2DAString("baseitems","RangedWeapon",iBaseItem) == "") {
        return TRUE;
      }
    }
    return FALSE;
...
struct ip_wildshape_bonus __new_ip_wildshape_bonus() {
  struct ip_wildshape_bonus s;
  s.ability0 = 0;
  s.ability1 = 0;
  s.ability2 = 0;
  s.ability3 = 0;
...
struct ip_wildshape_bonus IPWildShapeGetItemPropertyBonus(object oItem) {
  struct ip_wildshape_bonus s;
  s = __new_ip_wildshape_bonus();
  ...
  while (GetIsItemPropertyValid(ip)) {
    ...
    int iValue = GetItemPropertyInteger(ip,3);
    if(GetItemPropertyDurationType(ip) != DURATION_TYPE_PERMANENT) {
      ip = GetNextItemProperty(oItem);
      continue;
    }
    if(ipType == ITEM_PROPERTY_ABILITY_BONUS) {
      switch(GetItemPropertySubType(ip)) {
        case 0: s.ability0 = iValue; break;
        case 1: s.ability1 = iValue; break;
        case 2: s.ability2 = iValue; break;
        case 3: s.ability3 = iValue; break;
        case 4: s.ability4 = iValue; break;
        case 5: s.ability5 = iValue; break;
      }
    }
    if(ipType == ITEM_PROPERTY_AC_BONUS) {
      switch(GetBaseItemType(oItem)) {
        case BASE_ITEM_AMULET:
          s.AC_natural = iValue; break;
        case BASE_ITEM_ARMOR:
        case BASE_ITEM_BRACER:
          s.AC_armor = iValue; break;
        case BASE_ITEM_LARGESHIELD:
        case BASE_ITEM_SMALLSHIELD:
        case BASE_ITEM_TOWERSHIELD:
          s.AC_shield = iValue; break;
        case BASE_ITEM_GLOVES:
        case BASE_ITEM_RING:
        case BASE_ITEM_BELT:
        case BASE_ITEM_HELMET:
        case BASE_ITEM_CLOAK:
          s.AC_deflection = iValue; break;
        case BASE_ITEM_BOOTS:
          s.AC_dodge = iValue; break;
      }
    }
    ip = GetNextItemProperty(oItem);
  }
...
struct ip_wildshape_bonus IPSumBonuses(struct ip_wildshape_bonus a, struct ip_wildshape_bonus b) {
  struct ip_wildshape_bonus s;
  s.ability0 = a.ability0 + b.ability0;
  s.ability1 = a.ability1 + b.ability1;
  s.ability2 = a.ability2 + b.ability2;
  s.ability3 = a.ability3 + b.ability3;
  s.ability4 = a.ability4 + b.ability4;
  s.ability5 = a.ability5 + b.ability5;

  s.AC_natural = a.AC_natural;
  if(b.AC_natural > a.AC_natural)
    s.AC_natural = b.AC_natural;
  s.AC_armor = a.AC_armor;
  if(b.AC_armor > a.AC_armor)
    s.AC_armor = b.AC_armor;
  s.AC_shield = a.AC_shield;
  if(b.AC_shield > a.AC_shield)
    s.AC_shield = b.AC_shield;
  s.AC_deflection = a.AC_deflection;
  if(b.AC_deflection > a.AC_deflection)
    s.AC_deflection = b.AC_deflection;
  s.AC_dodge = a.AC_dodge;
  if(b.AC_dodge > a.AC_dodge)
    s.AC_dodge = b.AC_dodge;

  return s;
}

void IPWildShapeApplyBonusses(struct ip_wildshape_bonus s, object oNew) {
  AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyAbilityBonus(0, s.ability0), oNew);
  AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyAbilityBonus(1, s.ability1), oNew);
  AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyAbilityBonus(2, s.ability2), oNew);
  AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyAbilityBonus(3, s.ability3), oNew);
  AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyAbilityBonus(4, s.ability4), oNew);
  AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyAbilityBonus(5, s.ability5), oNew);

 int iAC = s.AC_natural + s.AC_armor + s.AC_shield + s.AC_deflection + s.AC_dodge;
 AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyACBonus(iAC), oNew);
