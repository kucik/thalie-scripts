...
// NPC se spawnou v nahodnych rozestupech +- 1m
...
void main()
{
    ...
    // zabrani opakovanemu spawnovani
    ...
    // pocet typu NPC nacteny z triggeru
    int nMaxTypes = GetLocalInt(OBJECT_SELF, "MAX_TYPES");
    if (nMaxTypes <= 0) nMaxTypes = 1; // bezpecny fallback
    ...
    for (iType = 1; iType <= nMaxTypes; iType++)
    {
        ...
        if (sResRef != "" && nCount > 0)
        {
            ...
            for (i = 0; i < nCount; i++)
            {
                // nahodne rozhozeni kolem waypointu (±1 metr)
                vector vRand = vBase;
                vRand.x += (Random(200) - 100) / 100.0; // -1.0 az +1.0
