...
void main()
{
    ...
    //--------------------------------------------------------------------------
    int nNumDice = GetThalieCaster(OBJECT_SELF,OBJECT_SELF,GetCasterLevel(OBJECT_SELF),FALSE)/2;
    if (nNumDice<1)
    {
        nNumDice = 1;
    }
    ...
    while (GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                nDamage    = GetReflexAdjustedDamage(nPotential, oTarget, GetSpellSaveDC()+GetThalieSpellDCBonus(OBJECT_SELF), SAVING_THROW_TYPE_ELECTRICITY);
